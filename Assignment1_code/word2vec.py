...
def naiveSoftmaxLossAndGradient(
    ...
    Imple`ment the naive softmax loss and gradients between a center word's 
    ...
    v_c = centerWordVec
    U = outsideVectors.T
    y_hat = softmax(U.T @ centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    gradCenterVec = (sigmoid(u_o @ v_c) - 1) * u_o
    gradOutsideVecs = np.zeros_like(U.T)
    gradOutsideVecs[o] += (sigmoid(u_o @ v_c) - 1) * v_c
    
    for k in negSampleWordIndices:
        u_k = U[:, k]
        loss -= np.log(sigmoid(-u_k @ v_c))
        gradCenterVec += sigmoid(u_k @ v_c) * u_k
        gradOutsideVecs[k] += sigmoid(u_k @ v_c) * v_c
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        l, gcv, gov = word2vecLossAndGradient(v_c, o, U.T, dataset)
        loss += l
        gradCenterVecs[c] += gcv
